...
function PutComponents(
  child_components: React.JSX.Element[],
  userId: string,
  it: number,
  dateScope: Date,
  ...
  while (it < sort_transactions.length && sort_transactions[it].createdAt >= dateScope) {
    ...
    child_components.push(
      <TransactionBox
        key={ts.id}
        transaction={ts}
        type={userId == ts.sellerId ? "sell" : "buy"}
...
const TransactionList = ({
  ...
  const { transactions, loading, error } = useGetQueryTransaction(
    filter,
    userId,
    transactionPerPage * (selectingPage - 1),
    transactionPerPage
  );
